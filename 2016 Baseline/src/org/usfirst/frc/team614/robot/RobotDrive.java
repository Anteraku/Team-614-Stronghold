...
public class RobotDrive implements MotorSafety {
  ...
  public static class MotorType {
    ...
    static final int kMidLeft_val = 4;
    static final int kMidRight_val = 5;
    /**
  ...
  /**
   * Constructor for RobotDrive with 6 motors specified with channel numbers.
   * Set up parameters for a four wheel drive system where all six motor pwm
   ...
   * @param midLeftMotor Mid Left motor channel number
   * @param midRightMotor Mid Right motor channel number
   */
  public RobotDrive(final int frontLeftMotor, final int frontRightMotor, final int rearLeftMotor,
      final int rearRightMotor, final int midLeftMotor, final int midRightMotor){
    ...
    m_midLeftMotor = new VictorSP(midLeftMotor);
    m_midRightMotor= new VictorSP(midRightMotor);
   ...
   *        the robot.
   */
  public RobotDrive(SpeedController frontLeftMotor, SpeedController frontRightMotor,
      SpeedController rearLeftMotor, SpeedController rearRightMotor, SpeedController midLeftMotor, SpeedController midRightMotor) {
...
//  }
  ...
  public void setLeftRightMotorOutputs(double leftOutput, double rightOutput) {
    ...
    if (m_midLeftMotor != null) {
    	m_midLeftMotor.set(limit(leftOutput) * m_maxOutput, m_syncGroup);
      }
   // m_midLeftMotor.set(-limit(rightOutput) * m_maxOutput, m_syncGroup);

    if (m_midRightMotor != null) {
    	m_midRightMotor.set(-limit(rightOutput) * m_maxOutput, m_syncGroup);
      }
    //m_midRightMotor.set(-limit(rightOutput) * m_maxOutput, m_syncGroup);
  ...
  public void setInvertedMotor(MotorType motor, boolean isInverted) {
    switch (motor.value) {
      ...
      case MotorType.kRearRight_val:
        ...
        break;
     /** case MotorType.kMidLeft_val;
     // m_midLeftMotor.setInverted(isInverted);
      //break;
    //  case MotorType.kMidRight_val;
     // m_midRightMotor.setInverted(isInverted);
     break;
   */ 
    }
  }
